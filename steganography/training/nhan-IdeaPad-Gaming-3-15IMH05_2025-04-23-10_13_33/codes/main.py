...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        if opt.cuda:
            ...
            concat_img = concat_img.cuda()
        with torch.no_grad():
            concat_imgv = concat_img  # concat_img as input of Hiding net
            cover_imgv = cover_img # cover_imgv as label of Hiding net
        ...
        debug_dir = os.path.join('debug_images', f'epoch_{epoch}', f'batch_{i}')
        os.makedirs(debug_dir, exist_ok=True)

        vutils.save_image(cover_img, os.path.join(debug_dir, str(epoch)+'_1_cover_img.png'), normalize=True)
        vutils.save_image(secret_img, os.path.join(debug_dir, str(epoch)+'_2_secret_img.png'), normalize=True)
        vutils.save_image(container_img.data, os.path.join(debug_dir, str(epoch)+'_3_container_img.png'), normalize=True)
        vutils.save_image(rev_secret_img.data, os.path.join(debug_dir, str(epoch)+'_4_rev_secret_img.png'), normalize=True)
        save_result_pic(this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img.data, epoch, i,
                        opt.testPics)
