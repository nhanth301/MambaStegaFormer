...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        vutils.save_image(cover_img, os.path.join(debug_dir, epoch+'_1_cover_img.png'), normalize=True)
        vutils.save_image(secret_img, os.path.join(debug_dir, epoch+'_2_secret_img.png'), normalize=True)
        vutils.save_image(container_img.data, os.path.join(debug_dir, epoch+'_3_container_img.png'), normalize=True)
        vutils.save_image(rev_secret_img.data, os.path.join(debug_dir, epoch+'_4_rev_secret_img.png'), normalize=True)
        save_result_pic(this_batch_size, cover_img, container_img.data, secret_img, rev_secret_img.data, epoch, i,
                        opt.testPics)
